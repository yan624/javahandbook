...
## 缓存一致性
缓存一致性的问题：在数据库中更新了数据，但是没同步到缓存中。

如何缓存一致性？

1. 先更新数据库，再更新缓存。缓存可能更新失败。
2. 先删除缓存，再更新数据库。并发时，在当前线程还没更新数据库之前，其它线程可能会读数据，这样又把老数据写回缓存了。
3. 先更新数据库，再删除缓存。缓存可能删除失败。
4. 延时双删：
5. 操作串行化：
	1. 先删缓存
	2. 将更新数据库的操作放入有序队列
	3. 如果从缓存中查不到数据，那么将后续查询数据库的操作也放入有序队列
	4. 面临的问题：
		1. 读请求积压，大量超时，导致数据库的压力：限流、熔断
		2. 如何避免大量请求积压：将队列水平拆分，提高并发量
		3. 。。。。。
